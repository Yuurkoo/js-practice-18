...
function getBookById(books, id) {
  ...
  // try {
  //   if (books === undefined) {
  //     let err = TypeError(`Книга з ID ${id} не знайдена!`);
  //     throw err;
  //   }
  //   return books;
  // } catch (e) {
...
function findEvenNumber(numbers) {
  ...
  try {
    ...
    if (evenNumber === undefined) {
      let error = Error("У масиві немає чисел, що діляться на 2 без остачі!");
      throw error;
    }
...
function validateUser(user) {
  ...
  // Перевіряємо, чи існує ім'я користувача,якщо ні викидуємо помилку з повідомленням "mail користувача не вказано!", а як причину вказуємо об'єкт user.
  ...
  try {
    if (!user) {
      console.log("Об'єкт користувача не вказано!");
    } else {
      if (!user.name) {
        console.log("Ім'я користувача не вказано!", user);
      }
      if (!user.email) {
        console.log("Email користувача не вказано!", user);
      }
...
function calculateSquareRoot(number) {
  ...
  try {
    if (typeof number !== "number") {
      const err = new Error("Аргумент має бути числом!");
      throw err;
    }
    if (number < 0) {
      const err1 = new Error("Число не повинно бути від'ємним!");
      throw err1;
    }
  ...
  try {
    for (let i = 0; i < data.length; i++) {
      if (typeof data[i] !== "number") {
        const err = new Error(`Елемент з індексом ${[i]} має бути числом!`);
        throw err;
      }
    }
...
function evaluateExpression(expression) {
  ...
  try {
    ...
    if (isNaN(result)) {
      const err = new Error(`Помилка при обробці даних`);
      throw err;
    }
    ...
    //
    //EvalError не спрацьовує тому залишив просто err
  }
}
